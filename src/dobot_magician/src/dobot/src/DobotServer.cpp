...
/*bool GetDeviceVersionService(uint8_t typeIndex, dobot::GetDeviceVersion::Request &req, dobot::GetDeviceVersion::Response &res)
{
    uint8_t majorVersion, minorVersion, revision;

    res.result = GetDeviceVersion(typeIndex, &majorVersion, &minorVersion, &revision);
    if (res.result == DobotCommunicate_NoError) {
...
int main(int argc, char **argv)
{
  ...
  /*  QString dobotPortName;
    QList<QSerialPortInfo> availablePorts = QSerialPortInfo::availablePorts();

    foreach(QSerialPortInfo portInfo, availablePorts) {
        if (portInfo.description().contains("Prolific")) {
            dobotPortName = portInfo.portName();
            break;
        }
    } */
