...
#include "dashgo_gazebo_plugins/gazebo_ros_dashgo.h"

namespace gazebo
{


GazeboRosDashgo::GazeboRosDashgo() : shutdown_requested_(false)
{
  // Initialise variables
  wheel_speed_cmd_[LEFT] = 0.0;
  wheel_speed_cmd_[RIGHT] = 0.0;
  //for restrict the update rate
  _pub_count = 0;
...
GazeboRosDashgo::~GazeboRosDashgo()
{
//  rosnode_->shutdown();
...
void GazeboRosDashgo::Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
{
  model_ = parent;
  if (!model_)
  {
  ...
  gazebo_ros_ = GazeboRosPtr(new GazeboRos(model_, sdf, "Dashgo"));
  sdf_ = sdf;
...
/*  if(prepareCliffSensor() == false)
  ...
  ROS_INFO_STREAM("GazeboRosDashgo plugin ready to go! [" << node_name_ << "]");
  update_connection_ = event::Events::ConnectWorldUpdateEnd(boost::bind(&GazeboRosDashgo::OnUpdate, this));

}


void GazeboRosDashgo::OnUpdate()
{
  /*
...
/*
void GazeboRosDashgo::motorPowerCB(const kobuki_msgs::MotorPowerPtr &msg)
{
...
*/
void GazeboRosDashgo::cmdVelCB(const geometry_msgs::TwistConstPtr &msg)
{
...
void GazeboRosDashgo::resetOdomCB(const std_msgs::EmptyConstPtr &msg)
{
  odom_pose_[0] = 0.0;
  odom_pose_[1] = 0.0;
  odom_pose_[2] = 0.0;
...
GZ_REGISTER_MODEL_PLUGIN(GazeboRosDashgo);
